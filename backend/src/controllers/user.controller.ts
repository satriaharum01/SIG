...
export class UserController {
  ...
  getByUuid = async (req: Request, res: Response) => {
    try {
      const data = await this.userService.getUserByUuid(req.params.uuid as string);
      res.json({ status: true, data });
    } catch (err: any) {
  ...
  update = async (req: Request, res: Response) => {
    try {
      const data = await this.userService.updateUser(req.params.uuid as string, req.body);
  ...
  delete = async (req: Request, res: Response) => {
    try {
      await this.userService.deleteUser(req.params.uuid as string);
